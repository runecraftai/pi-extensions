...
describe("context bar", () => {
  it("renders smart, warm, and dumb zones", () => {
    ...
    assert.ok(renderContextBar(theme, 55, 55_000, 100_000, 40).includes("⚠️"));
    assert.ok(renderContextBar(theme, 80, 80_000, 100_000, 40).includes("🧟"));
    assert.ok(renderContextBar(theme, 25, 25_000, 100_000, 40).includes("│"));
    assert.ok(renderContextBar(theme, 55, 55_000, 100_000, 40).includes("55.0%"));
    assert.ok(renderContextBar(theme, 80, 80_000, 100_000, 40).includes("80.0%"));
  });

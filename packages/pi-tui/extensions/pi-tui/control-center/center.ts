...
import type { FooterSegmentKey, FooterZone, PiTuiConfig, DiffViewMode } from "../config.ts";
import { loadConfig, saveConfig } from "../config.ts";
import { openLazyGit } from "./lazygit.ts";

/* ── Tab definitions ── */

type Tab = "overview" | "context" | "appearance" | "renderers" | "settings";
const TABS: Tab[] = ["overview", "context", "appearance", "renderers", "settings"];
...
function cycleDiffMode(config: PiTuiConfig): PiTuiConfig {
  const order: DiffViewMode[] = ["auto", "split", "unified"];
  const idx = order.indexOf(config.renderers.diff.mode);
  const mode = order[(idx + 1) % order.length]!;
  return { ...config, renderers: { ...config.renderers, diff: { ...config.renderers.diff, mode } } };
}
...
function buildRendererItems(config: PiTuiConfig): SettingItem[] {
  ...
  items.push({ id: "rend:diffMode", label: "Diff mode", currentValue: config.renderers.diff.mode });
...
function buildOverviewItems(config: PiTuiConfig): SettingItem[] {
  ...
  return [
    ...
    { id: "ov:diffMode", label: "Diff mode", currentValue: config.renderers.diff.mode },
  ];
}
...
class ControlCenterUi {
  ...
  private renderOverview(): void {
    ...
    const lines: string[] = [
      ...
      `  Diff mode: ${this.config.renderers.diff.mode}`,
      "",
  ...
  pi.registerCommand("pi-tui", {
    ...
    handler: async (args, ctx: ExtensionContext) => {
      ...
      if (subcommand !== "" && subcommand !== "reload") {
        ctx.ui.notify(
          `Unknown /pi-tui subcommand: "${subcommand}". Available: reload, context, git, renderers, appearance, settings`,
          "warning",
        );
        return;
      }
